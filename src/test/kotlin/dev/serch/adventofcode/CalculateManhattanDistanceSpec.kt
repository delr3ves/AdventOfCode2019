...
internal class CalculateManhattanDistanceSpec : StringSpec({
    ...
    "test samples with time should find the solution" {
        forall(
                row(
                        listOf("R8", "U5", "L5", "D3"),
                        listOf("U7", "R6", "D4", "L4"),
            ...
            calculateManhattanDistance.takingTimeIntoAccount(wire1, wire2) shouldBe expected
        }
    }

    "test with input and time should solve the problem" {
        val input = javaClass.classLoader.getResource("ManhattanCalculatorInput.txt")
                .readText()
                .split("\n")
                .map { it.split(",") }
        calculateManhattanDistance.takingTimeIntoAccount(input[0], input[1]) shouldBe 122514
    }
})